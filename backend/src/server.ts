...
io.on("connection", (socket: ServerSocket) => {
  ...
  socket.on("joinGame", (data: { gameId: string }, callback) => {
    ...
    if (joinResult.success && joinResult.playerRole) {
      ...
      console.log(
        `Socket ${socket.id} successfully joined game ${gameId} as ${joinResult.playerRole}`
      );
      if (typeof callback === "function") {
        callback({
          success: true,
      ...
      console.log(
        `Socket ${socket.id} failed to join game ${gameId}: ${joinResult.message}`
      );
      if (typeof callback === "function") {
  ...
  socket.on("paddleMove", (data: PaddleMovePayload) => {
    ...
    if (!gameId || !playerRole) {
      console.warn(
        `Socket ${socket.id} sent paddleMove without being in a game or assigned a role.`
      );
      return;
    }
    ...
    if (data.playerId !== playerRole) {
      console.warn(
        `Socket ${socket.id} (role: ${playerRole}) tried to move paddle for ${data.playerId}. Denied.`
      );
      return;
    }
    ...
    if (!game) {
      console.warn(
        `Game ${gameId} not found for paddleMove from socket ${socket.id}`
      );
      return;
    }
    ...
    if (playerRole === "player1") {
      ...
      console.warn(
        `Invalid playerRole ${playerRole} for socket ${socket.id} in game ${gameId}`
      );
      return;
    }
  ...
  socket.on("disconnect", () => {
    ...
    if (disconnectInfo) {
      ...
      console.log(
        `Player (socket ${socket.id}, role ${socket.data.playerRole}) left game ${gameId}.`
      );
...
setInterval(() => {
  gameManager.getActiveGames().forEach((game, gameId) => {
    ...
    if (currentGameState.isGameOver && !wasGameOver) {
      console.log(
        `Game ${gameId} ended. Winner: ${currentGameState.winner}. Broadcasting 'gameOver' event.`
      );
